	...
	torch::Tensor &residual)
{
	...
	T *dResidual = (T *)(residual.storage().data());
	memset(dResidual, 0, sizeof(T) * numResiduals * numDimJ);

	for (int i = 0; i < numVar; ++i)
	{
		int indicesDim = indices[i].size(1);
		long *dIndices = (long *)(indices[i].storage().data());
		auto &J = jacobians[i];
		auto &P = p[i];
		int numDimP = P.size(1);
		T *dP = (T *)(P.storage().data());
		T *dJ = (T *)(J.storage().data());
...
#pragma omp parallel for
		for (int j = 0; j < numResiduals; ++j)
		{
			for (int dim = 0; dim < indicesDim; ++dim)
			{
				...
				{
					T *ptrJ = dJ + numDimP * ((numResiduals * k + j) * indicesDim + dim);
	...
	std::vector<torch::Tensor> &jtrs)
{
	...
	for (int i = 0; i < numVar; ++i)
	{
		auto &jtr = jtrs[i];
		T *dJtr = (T *)(jtr.storage().data());
		T *dD = (T *)(diagonal[i].storage().data());
		T *dP = (T *)(p[i].storage().data());
	...
	std::vector<torch::Tensor> &B)
{
	...
	for (int i = 0; i < numVar; ++i)
	{
		...
		auto dA = (T *)(A[i].storage().data());
		auto dX = (T *)(X[i].storage().data());
		auto dB = (T *)(B[i].storage().data());
...
#pragma omp parallel for
	for (int i = 0; i < jacobians.size(); ++i)
	{
		// printf("i %d\n", i);
		int indicesDim = indices[i].size(1);
		auto dIndices = (long *)(indices[i].storage().data());
		auto dColumnNorm = (T *)(jacobianScale[i].storage().data());
		auto dJacobian = (T *)(jacobians[i].storage().data());
